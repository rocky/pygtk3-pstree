...
def ReadLoadAvg(path):
    ...
    with open(path) as fd:
        for line in fd.readlines():
            ...
            if len(fields) < 5:
                raise TypeError("Cannot parse loadavg, need 5 fields: "
                                + line.strip())

            process = fields[3].split("/")

            if len(process) != 2:
                raise TypeError("Missing / in field 3 of load average " +
                                 line.stip())
            ...
            loadavg = LoadAvg(Last1Min, Last5Min, Last15Min, ProcessRunning,
                              ProcessTotal, LastPid)
            pass
        pass
    return loadavg
